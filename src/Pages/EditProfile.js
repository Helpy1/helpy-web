...
const EditProfile = () => {
    ...
    const [selectedFilters, setSelectedFilters] = useState({
        ...
        heightRange: [100, 200],
        heightinInches : userData.heightInInches,
        language: "English",
        bills: [],
        kids: "",
        ...
        images: [
            userData.imageData.Post_1 || null,
            userData.imageData.Post_2 || null,
            userData.imageData.Post_3 || null,
            userData.imageData.Post_4 || null,
            userData.imageData.Post_5 || null,
            userData.imageData.Post_6 || null,
            userData.imageData.Post_7 || null,
            userData.imageData.Post_8 || null,
        ]
    });
    console.log(userData.imageData[0])
        ...
        const fetchBillsData = async () => {
            try {
                ...
                if (data && Array.isArray(data)) {
                    ...
                    // Map userData.bills names to their corresponding IDs
                    const matchedBills = data
                        .filter(bill => userData.bills.includes(bill.name)) // Match names
                        .map(bill => bill.id); // Get corresponding IDs

                    setSelectedFilters(prevState => ({
                        ...prevState,
                        bills: matchedBills // Set the matched IDs
                    }));
                } else {
                    console.error("Invalid data format:", data);
                }
    ...
    const uploadUserImage = async () => {
        ...
        selectedFilters.images.forEach((image, imgIndex) => {
            ...
            if (image?.file) {
                ...
                const fileName = `${timestamp}_Post_${imgIndex + 1}`; // Sequentially index the images
        ...
        try {
            ...
            const result = uploadedImageData.map((image, imgIndex, index) => ({

                key: `Post_${imgIndex + 1}`, // Sequential key based on response order
    ...
    const cmToInches = (cm) => {
        ...
        const value = parseFloat(cm); // Ensure it's a number
        if (isNaN(value)) {
    ...
    return (
        <div>
            ...
            <div className="relative">
                ...
                {/* UI remains fully visible and interactive */}
                <div>
                            ...
                            <div className="w-full bg-white p-4 rounded-lg shadow-md">
                                ...
                                <div className="h-cover">
                                    <div className="text-sm font-medium">
                                        <span className="text-sm block">{`Height range: ${cmToInches(selectedFilters.heightRange[0])} inches - ${cmToInches(selectedFilters.heightRange[1])} inches`}</span>
                                    </div>
                                    <div className="my-4">
                                        <Slider
                                            range
                                            min={100}
                                            max={200}
                                            value={selectedFilters.heightInInches}
